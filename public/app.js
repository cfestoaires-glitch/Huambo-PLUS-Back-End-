...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // CRIAÇÃO AUTOMÁTICA DA CONTA DE ADMINISTRADOR
    // ==========================================
    // Podes alterar para o teu e-mail e password pretendidos:
    const ADMIN_EMAIL = 'aires.admin@huamboplus.com'; 
    const ADMIN_PASS = 'AdminHuambo2026*';
    ...
    signupBtn.addEventListener('click', async () => {
        ...
        if (role === 'provider') {
            ...
            try {
                ...
                const uploadSelfie = await supabaseClient.storage.from('images').upload(selfiePath, selfieFile);
                if (uploadSelfie.error) throw uploadSelfie.error;
                selfieUrl = uploadSelfie.data.path;

                const uploadBi = await supabaseClient.storage.from('images').upload(biPath, biFile);
